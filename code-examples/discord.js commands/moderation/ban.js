		...
		toBan.send(`You have been banned from **${message.guild.name}** by **${message.author.tag}** because of "${reason}"`).catch((e) => {});
		await message.guild.members.ban(toBan.id, { reason: `Banned by ${message.author.tag} | Reason: ${reason}` })
			.then((user) => {
				message.channel.send(`Success! **${user.tag}** was banned for "${reason || "[No Reason Given]"}"`)
			})
